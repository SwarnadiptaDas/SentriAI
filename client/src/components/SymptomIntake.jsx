...
const SymptomIntake = ({ onSubmit, preliminaryResult }) => {
  ...
  const handleVoiceSubmit = () => {
    ...
    if (finalTranscript.trim().length > 5) {
      ...
      setError('We didn\'t catch that. Please speak a bit more.');
    }
  };
    ...
    <div className="glass-card flex flex-col items-center p-8 md:p-12 max-w-2xl w-full text-center animate-[fadeSlideIn_0.5s_ease]">
      ...
      {!isManualMode ? (
        <div className="flex flex-col items-center w-full">
          ...
          <div className="flex gap-4 w-full mt-8">
            ...
            <button 
              ...
              disabled={isListening || (transcript.length === 0 && interimTranscript.length === 0)}
            >
              Analyze Symptoms
            </button>
          </div>
        ...
        <div className="flex flex-col w-full">
          <textarea 
            className="w-full bg-navy-900/60 border border-slate-700/50 rounded-xl p-5 text-lg text-slate-200 min-h-[200px] outline-none focus:border-mint/50 focus:ring-1 focus:ring-mint/50 transition-all shadow-inner"
